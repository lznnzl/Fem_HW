...
class BeamSolve():
    ...
    def read_3d3s(self,filename):
        ...
        for i in range(len(eledata)):
            if eledata[i,2] == 17:
                ele_info_tp[i] = eledata[i,3] - 1
            elif eledata[i,2] == 21:
                ele_info_tp[i] = eledata[i,3] - 1 + I_num
            else:
                print(eledata[0,2])
                raise ValueError('杆件截面类型溢出')
    ...
    def loadSet(self):
        ...
        # 计算过程
        loaddata = pd.read_excel('dataSet.xlsx',sheet_name='节点荷载').values
    ...
    def constraintSet(self):
        ...
        # 参数输入
        free_num = self.free_num
        temp = pd.read_excel('dataSet.xlsx', sheet_name='边界条件').values
    ...
    def freedomSet(self):
        ...
        # 数据读取
        temp = pd.read_excel('dataSet.xlsx', sheet_name='杆端释放').values
    ...
    def nolinear_NR(self,times = 10, err = 0.01):
        ...
        for i in range(times_load):
            ...
            for j in range(50):
                ...
                vec_force_add = self.cal_node_force_add(step_ele_force,now_node)

                '更新当前力及力偏差'
                now_force += vec_force_add
                now_force_err = now_tag_load - now_force
    ...
    def nolinear_Arc_len(self, method = 0):
        '采用弧长法进行计算'
        '这里先采用超平面约束 考虑超平面的更新'
        ...
        for i in range(times_load):
            ...
            rec_dis[i,0] = now_lamda
            rec_dis[i,1] = now_mat_dis[10,1]
